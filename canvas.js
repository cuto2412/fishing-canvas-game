...
let time = 89;
let score = 0;
// lưu highScore bằng local Storage
if(localStorage.getItem("highScore0") == null)
...
let topScore = [
  parseInt(localStorage.getItem("highScore0")),
  parseInt(localStorage.getItem("highScore1")),
  parseInt(localStorage.getItem("highScore2"))
];

topScore.sort((a,b) => a-b);

// localStorage.setItem("highScore0",topScore);
let keyLog = "";
...
//nhạc nền
// let bkAudio=document.getElementById("bkgroundAudio");
let sound_end=new Audio();
sound_end.src="assets/sound_endGame.mp3";
// bkAudio.play();
//hiệu ứng âm thanh và thay click chuột bằng nhấn phím cách để chạy thanh câu
let sound= new Audio();
sound.src="assets/amthanh.mp3";
let sound_fishing=new Audio();
sound_fishing.src="assets/sound_fishing.mp3";
let sound_ended=new Audio();
sound_ended.src="assets/AWW.mp3";
...
document.addEventListener("keypress", function (e) {
  ...
  // if(e.key=='p')
  //   bkAudio.pause(); //phím tắt bật/tắt nhạc
  // if(e.key=='o')
  //   bkAudio.play();
});
...
function gameTimer() {
  ...
  let interval = setInterval(function () {
    ...
    if (sec === 0) {
      ...
      // Best Score
    // bScore = parseInt(localStorage.getItem("highScore"));
    if(topScore[0] < score){
      topScore[0] = score;
      topScore.sort((a,b) => a-b);
      localStorage.setItem("highScore0",topScore[0]);
      localStorage.setItem("highScore1",topScore[1]);
      localStorage.setItem("highScore2",topScore[2]);
    }
...
function init() {
  ...
  // score
  if (fishCaught < 1) {
    context.drawImage(number0, 540, 20);
    context.drawImage(number0, 580, 20);
    context.drawImage(number0, 620, 20);
    context.drawImage(number0, 660, 20);
    context.drawImage(number0, 700, 20);
  }
  ...
  if (!timer) {   
    ...
    context.drawImage(number3, 160, 20);
    context.drawImage(number0, 200, 20);
  }
  //test nhân vật
// var character1;
  let characters = new Image();
  characters.src = "assets/nhan_vat.png";
  function characterSelection(){
    //context.drawImage(ảnh,toaDoX,toaDoY);
    context.drawImage(characters, innerWidth-330, innerHeight-340,350,250);//vẽ nhân vật 1 ở vị trí x y 
  }
 characterSelection();
  if (casting === false) {
    // fishing rod init
  ...
  // red
  context.fillStyle = "rgba(219, 0, 18, 1)";
  context.fillRect(window.innerWidth - 150, window.innerHeight - 400, 50, 10);
  context.fillRect(window.innerWidth - 280, window.innerHeight - 400, 50, 10);
  ...
  // context.drawImage(pufferFish,800,1000);
...
//in điểm cao
function highScoreDecider(placement,i) {
  let stringScore = topScore[i].toString();
  let highScoreDecider=parseInt(stringScore[placement - 1]);
  return numberArray[highScoreDecider];
}
...
let dx = 5;
let dy = 30;
//bảng xếp hạng điểm cao đang viết tiếp
...
function animate() {
  ...
  //in highScore 1
  context.drawImage(number1,1000,20,50,50);
  context.drawImage(number2,1000,70,50,50);
  context.drawImage(number3,1000,120,50,50);
  if (topScore[2] >= 1000) {  
    context.drawImage(highScoreDecider(4,2), 1030, 20,50,50);
    context.drawImage(highScoreDecider(3,2), 1070, 20,50,50);
    context.drawImage(highScoreDecider(2,2), 1110, 20,50,50);
    context.drawImage(highScoreDecider(1,2), 1150, 20,50,50);
  } else if (topScore[2] >= 100) {
    context.drawImage(highScoreDecider(1,2), 1070, 20,50,50);
    context.drawImage(highScoreDecider(2,2), 1110, 20,50,50);
    context.drawImage(highScoreDecider(3,2), 1150, 20,50,50);
  } else if (topScore[2] >= 10) {
    context.drawImage(highScoreDecider(1,2), 1110, 20,50,50);
    context.drawImage(highScoreDecider(2,2), 1150, 20,50,50);
  } else if (topScore[2] > 0) {
    context.drawImage(highScoreDecider(1,2), 1150, 20,50,50);
  } else {
    context.drawImage(number0, 1150, 20,50,50);
  }
  // điểm top2
  if (topScore[1] >= 1000) {  
    context.drawImage(highScoreDecider(4,1), 1030, 70,50,50);
    context.drawImage(highScoreDecider(3,1), 1070, 70,50,50);
    context.drawImage(highScoreDecider(2,1), 1110, 70,50,50);
    context.drawImage(highScoreDecider(1,1), 1150, 70,50,50);
  } else if (topScore[1] >= 100) {
    context.drawImage(highScoreDecider(1,1), 1070, 70,50,50);
    context.drawImage(highScoreDecider(2,1), 1110, 70,50,50);
    context.drawImage(highScoreDecider(3,1), 1150, 70,50,50);
  } else if (topScore[1] >= 10) {
    context.drawImage(highScoreDecider(1,1), 1110, 70,50,50);
    context.drawImage(highScoreDecider(2,1), 1150, 70,50,50);
  } else if (topScore[1] > 0) {
    context.drawImage(highScoreDecider(1,1), 1150, 70,50,50);
  } else {
    context.drawImage(number0, 1150, 70,50,50);
  }
  //điểm top3
  if (topScore[0] >= 1000) {  
    context.drawImage(highScoreDecider(4,0), 1030, 120,50,50);
    context.drawImage(highScoreDecider(3,0), 1070, 120,50,50);
    context.drawImage(highScoreDecider(2,0), 1110, 120,50,50);
    context.drawImage(highScoreDecider(1,0), 1150, 120,50,50);
  } else if (topScore[0] >= 100) {
    context.drawImage(highScoreDecider(1,0), 1070, 120,50,50);
    context.drawImage(highScoreDecider(2,0), 1110, 120,50,50);
    context.drawImage(highScoreDecider(3,0), 1150, 120,50,50);
  } else if (topScore[0] >= 10) {
    context.drawImage(highScoreDecider(1,0), 1110, 120,50,50);
    context.drawImage(highScoreDecider(2,0), 1150, 120,50,50);
  } else if (topScore[0] > 0) {
    context.drawImage(highScoreDecider(1,0), 1150, 120,50,50);
  } else {
    context.drawImage(number0, 1150, 120,50,50);
  }
  //in điểm
  if (score >= 10000) {
    context.drawImage(numberDecider(5), 540, 20);
    context.drawImage(numberDecider(4), 580, 20);
    context.drawImage(numberDecider(3), 620, 20);
    context.drawImage(numberDecider(2), 660, 20);
    context.drawImage(numberDecider(1), 700, 20);
  } else if (score >= 1000) {
    context.drawImage(number0, 540, 20);
    context.drawImage(numberDecider(4), 580, 20);
    context.drawImage(numberDecider(3), 620, 20);
    ...
    context.drawImage(number0, 540, 20);
    context.drawImage(number0, 580, 20);
    context.drawImage(numberDecider(1), 620, 20);
    context.drawImage(numberDecider(2), 660, 20);
    context.drawImage(numberDecider(3), 700, 20);
  } else if (score >= 10) {
    context.drawImage(number0, 540, 20);
    context.drawImage(number0, 580, 20);
    context.drawImage(number0, 620, 20);
    context.drawImage(numberDecider(1), 660, 20);
    context.drawImage(numberDecider(2), 700, 20);
  } else if (score > 0) {
    context.drawImage(number0, 540, 20);
    context.drawImage(number0, 580, 20);
    context.drawImage(number0, 620, 20);
  ...
  if (timer) {     // sửa hiển thị time thành dạng phút và tăng time 60s->90s
    ...
    if (stringTime.length < 2) {
      ...
      if(time >= 60){
        let lpl =parseInt(stringTime[0])-6;
        // stringTime[0]=lpl.toString();
        // console.log(stringTime[0]+" "+lpl );
        context.drawImage(number1, 80, 20);
        context.drawImage(colon, 120, 20);
        context.drawImage(numberArray[lpl], 160, 20);
...
var nen = document.getElementById("bgr");
btn1.onclick = function() {
nen.style.backgroundImage = "url(assets/bgr.jpeg)";
}
btn2.onclick = function() { 
  nen.style.backgroundImage = "url(assets/bgr2.jpeg)";
}